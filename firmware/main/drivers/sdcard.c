...
esp_err_t sdcard_spi_init() {
    ...
    esp_err_t ret =
        spi_bus_initialize(SPI2_HOST, &sdcard_spi_config, SPI_DMA_CH_AUTO);
    if(ret != ESP_OK) {
...
esp_err_t sdcard_mount(sdmmc_card_t** out_card) {
    ...
    esp_err_t ret = esp_vfs_fat_sdspi_mount(
        SD_CARD_MOUNT_PATH, &host, &slot_config, &mount_config, out_card);
...
static int32_t build_path(const char* filename, char* out_path,
                          size_t out_path_len) {
    if(filename == NULL || out_path == NULL)
        return 1;
    int32_t n =
        snprintf(out_path, out_path_len, "%s/%s", SD_CARD_MOUNT_PATH, filename);
