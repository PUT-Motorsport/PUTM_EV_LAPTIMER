...
static Config config_local;

static bool ip_refresh_flag = false;

/**
 * @brief Mutex used by data_get_handler to ensure that it can read local data
 * safely
 */
...
static esp_err_t data_get_handler(httpd_req_t* req) {
    ...
    if(xSemaphoreTake(data_mutex, portMAX_DELAY)) {
        laptime_current.convert_string_full(laptime_current_str,
        ...
        if(id < 0 || id >= DRIVER_MAX_COUNT)
            id = 0;
        root["current_driver_tag"] = config_local.driver_list.list[id];
        root["current_driver_id"] = id;

        laptime_current.convert_string_penalty(penalty_time_str,
                                               sizeof(penalty_time_str));
        root["penalty_time"] = penalty_time_str;

        snprintf(penalty_count_str, sizeof(penalty_count_str), "%u",
        ...
        if(xSemaphoreTake(laptime_lists_mutex, portMAX_DELAY)) {
            ...
            JsonArray driver_lap_count_arr =
                root["driver_lap_count"].to<JsonArray>();
            JsonArray driver_pen_time_arr =
                root["driver_pen_time"].to<JsonArray>();
            JsonArray driver_pen_oc_arr = root["driver_pen_oc"].to<JsonArray>();
            JsonArray driver_pen_doo_arr =
                root["driver_pen_doo"].to<JsonArray>();

            for(int i = 0; i <= config_local.driver_list.driver_count; i++) {
                drivers_arr.add(config_local.driver_list.list[i]);

                laptime_list_driver[i].convert_string_time(
                    laptime_current_str, sizeof(laptime_current_str));
                driver_best_arr.add(laptime_current_str);

                driver_lap_count_arr.add(laptime_list_driver[i].count - 1);

                laptime_list_driver[i].convert_string_penalty(
                    penalty_time_str, sizeof(penalty_time_str));
            ...
            JsonArray last_driver_tag_arr =
                root["last_driver_tag"].to<JsonArray>();
            JsonArray last_driver_id_arr =
                root["last_driver_id"].to<JsonArray>();
            JsonArray top_driver_tag_arr =
                root["top_driver_tag"].to<JsonArray>();
            ...
            for(int i = 0; i < LAPTIME_LIST_SIZE_WIFI; i++) {
                // Last
                laptime_list_last[i].convert_string_full(
                    laptime_current_str, sizeof(laptime_current_str));
                ...
                if(drv_id < 0 || drv_id >= DRIVER_MAX_COUNT)
                    drv_id = 0;
                last_driver_tag_arr.add(config_local.driver_list.list[drv_id]);
                last_driver_id_arr.add(drv_id);

                laptime_list_last[i].convert_string_penalty(
                    penalty_time_str, sizeof(penalty_time_str));
                ...
                // Top
                laptime_list_top[i].convert_string_full(
                    laptime_current_str, sizeof(laptime_current_str));
                ...
                if(drv_id < 0 || drv_id >= DRIVER_MAX_COUNT)
                    drv_id = 0;
                top_driver_tag_arr.add(config_local.driver_list.list[drv_id]);
...
static esp_err_t laptimes_csv_get_handler(httpd_req_t* req) {
    ...
    httpd_resp_set_hdr(req, "Content-Disposition",
                       "attachment; filename=\"laptimes.csv\"");

    // Header
    httpd_resp_sendstr_chunk(req, "Number,Laptime [mm:ss:ms], Driver, Penalty "
                                  "Time [mm:ss],OC,DOO,Date,Hour\n");
    ...
    if(xSemaphoreTake(laptime_lists_mutex, portMAX_DELAY)) {
        ...
        for(int i = 0; i < LAPTIME_LIST_SIZE_WIFI; i++) {
            laptime_list_last[i].convert_string_full(time_str,
                                                     sizeof(time_str));
            laptime_list_last[i].convert_string_penalty(pen_str,
                                                        sizeof(pen_str));
            ...
            snprintf(line, sizeof(line), "%s,%s,%s,%u,%u,%s,%s\n", time_str,
                     config_local.driver_list.list[drv_id], pen_str,
                     laptime_list_last[i].oc_count,
                     laptime_list_last[i].doo_count, laptime_list_last[i].date,
...
static esp_err_t drivers_csv_get_handler(httpd_req_t* req) {
    ...
    httpd_resp_sendstr_chunk(
        req, "Driver,Best laptime,Lap count,Penalty time sum,OC sum,DOO sum\n");
    ...
    if(xSemaphoreTake(laptime_lists_mutex, portMAX_DELAY)) {
        ...
        for(int i = 1; i <= config_local.driver_list.driver_count; i++) {
            laptime_list_driver[i].convert_string_time(time_str,
                                                       sizeof(time_str));
            laptime_list_driver[i].convert_string_penalty(pen_str,
                                                          sizeof(pen_str));

            snprintf(line, sizeof(line), "%s,%s,%u,%s,%u,%u\n",
                     config_local.driver_list.list[i], time_str,
                     laptime_list_driver[i].count, pen_str,
                     laptime_list_driver[i].oc_count,
                     laptime_list_driver[i].doo_count);
            httpd_resp_sendstr_chunk(req, line);
        }
...
static esp_err_t config_get_handler(httpd_req_t* req) {
    JsonDocument root;
    root["two_gate_mode"] = config_local.two_gate_mode;
    // Return current wifi mode as number (0 for AP, 1 for STA)
    root["wifi_mode"] = config_local.wifi_mode;
    root["wifi_ssid"] = config_local.wifi_ssid;
    root["wifi_password"] = config_local.wifi_password;
    root["time_set"] = config_local.time_set;
    root["date_set"] = config_local.date_set;

    JsonArray drivers = root["driver_list"].to<JsonArray>();
    // Driver list starts from 1, index 0 is placeholder
    for(int i = 1; i <= config_local.driver_list.driver_count; i++) {
        drivers.add(config_local.driver_list.list[i]);
    }
    httpd_resp_set_type(req, "application/json");
    HttpdChunkWriter writer(req);
    serializeJson(root, writer);
...
static esp_err_t config_post_handler(httpd_req_t* req) {
    ...
    if(root["two_gate_mode"].is<bool>())
        config_temp.two_gate_mode = root["two_gate_mode"];

    if(root["wifi_mode"].is<int>()) {
        config_temp.wifi_mode =
            (root["wifi_mode"].as<int>() == 1) ? WIFI_MODE_STA : WIFI_MODE_AP;
    } else if(root["wifi_mode"].is<bool>()) {
        config_temp.wifi_mode =
            root["wifi_mode"].as<bool>() ? WIFI_MODE_STA : WIFI_MODE_AP;
    }

    if(root["wifi_ssid"].is<const char*>()) {
        strncpy(config_temp.wifi_ssid, root["wifi_ssid"].as<const char*>(),
                sizeof(config_temp.wifi_ssid) - 1);
        config_temp.wifi_ssid[sizeof(config_temp.wifi_ssid) - 1] = '\0';
    }

    if(root["wifi_password"].is<const char*>()) {
        strncpy(config_temp.wifi_password,
                root["wifi_password"].as<const char*>(),
                sizeof(config_temp.wifi_password) - 1);
        config_temp.wifi_password[sizeof(config_temp.wifi_password) - 1] = '\0';
    }

    if(root["time_set"].is<const char*>()) {
        strncpy(config_temp.time_set, root["time_set"].as<const char*>(),
                sizeof(config_temp.time_set) - 1);
        config_temp.time_set[sizeof(config_temp.time_set) - 1] = '\0';
    }

    if(root["date_set"].is<const char*>()) {
        strncpy(config_temp.date_set, root["date_set"].as<const char*>(),
                sizeof(config_temp.date_set) - 1);
        config_temp.date_set[sizeof(config_temp.date_set) - 1] = '\0';
    }

    system_set_time(config_temp.time_set, config_temp.date_set);
    ...
    if(root["driver_list"].is<JsonArray>()) {
        ...
        config_temp.driver_list.driver_count = count;

        // Clear existing list beyond count if needed, or just overwrite
        // We overwrite from index 1
        for(int i = 0; i < count; i++) {
            if(drivers[i].is<const char*>()) {
                strncpy(config_temp.driver_list.list[i + 1],
                        drivers[i].as<const char*>(), DRIVER_TAG_LENGTH - 1);
                config_temp.driver_list.list[i + 1][DRIVER_TAG_LENGTH - 1] =
                    '\0';
            }
        }
    }
...
static httpd_handle_t start_webserver(void) {
    ...
    if(httpd_start(&server, &config) == ESP_OK) {
        ...
        httpd_uri_t data = {.uri = "/api/data",
                            .method = HTTP_GET,
                            .handler = data_get_handler,
                            .user_ctx = NULL};
        httpd_register_uri_handler(server, &data);

        httpd_uri_t csv = {.uri = "/api/csv",
                           .method = HTTP_GET,
                           .handler = laptimes_csv_get_handler,
                           .user_ctx = NULL};
        httpd_register_uri_handler(server, &csv);

        httpd_uri_t drivers_csv = {.uri = "/api/drivers_csv",
                                   .method = HTTP_GET,
                                   .handler = drivers_csv_get_handler,
                                   .user_ctx = NULL};
        httpd_register_uri_handler(server, &drivers_csv);

        httpd_uri_t config_get = {.uri = "/api/config",
                                  .method = HTTP_GET,
                                  .handler = config_get_handler,
                                  .user_ctx = NULL};
        httpd_register_uri_handler(server, &config_get);

        httpd_uri_t config_post = {.uri = "/api/config",
                                   .method = HTTP_POST,
 ...
 *  then in loop updates values displayed on http website with values received
 * from laptimer_task
 */
    ...
    for(;;) {
        ...
        if(wifi_reset_flag != WIFI_NO_RESET) {
            ...
            if(wifi_reset_flag == WIFI_RESET_CONFIG) {
                if(xSemaphoreTake(config_mutex, portMAX_DELAY) == pdTRUE) {
                    snprintf(wifi_ssid_local, sizeof(wifi_ssid_local),
                             config_main.wifi_ssid);
                    snprintf(wifi_password_local, sizeof(wifi_password_local),
                             config_main.wifi_password);
            ...
            // Reinit with safe values
                ...
                snprintf(wifi_ssid_local, sizeof(wifi_ssid_local), "%s",
                         WIFI_SSID_DEFAULT);
                snprintf(wifi_password_local, sizeof(wifi_password_local), "%s",
                         WIFI_PASSWORD_DEFAULT);
                wifi_mode_flag = WIFI_MODE_DEFAULT;
            }

            if(wifi_restart(wifi_mode_flag, wifi_ssid_local,
                            wifi_password_local) == ESP_OK) {
            ...
            if(xQueueReceive(laptime_current_queue_wifi, &laptime_current_temp,
                             0) == pdTRUE) {
                if(xSemaphoreTake(data_mutex, portMAX_DELAY)) {
