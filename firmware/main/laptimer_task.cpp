...
/// @brief Time penalty added to current laptime by LAP_DOO_PIN and LAP_OC_PIN
/// in 1/100s
const uint32_t DOO_TIME_PENALTY = 200;
const uint32_t OC_TIME_PENALTY = 1000;

/**
 * @brief Used to store current laptime that is readed from timer in main loop
 * and saved in ISR
 */
 ...
 */
static esp_err_t
    laptime_save_top(Laptime laptime,
                     std::array<Laptime, LAPTIME_LIST_SIZE_LOCAL>& list_top) {
...
static esp_err_t
    laptime_save_last(Laptime laptime,
                      std::array<Laptime, LAPTIME_LIST_SIZE_LOCAL>& list_last) {
...
static esp_err_t
    laptime_save_driver(Laptime laptime,
                        std::array<Laptime, DRIVER_MAX_COUNT>& list_driver) {
    if(laptime.time < list_driver.at(laptime.driver_id).time ||
       list_driver.at(laptime.driver_id).time == 0)
...
static void laptime_save_uart(Laptime laptime) {
    ...
    laptime.convert_string_full(laptime_saved_str, sizeof(laptime_saved_str));
    printf("%s\n", laptime_saved_str);
}
...
static void penalty_check(Laptime* laptime) {
    ...
    switch(doo_press.state =
               button_hold(gpio_get_level(LAP_DOO_PIN), doo_press)) {
    case BTN_HOLD_ACTION:
        if(laptime->penalty_time >= DOO_TIME_PENALTY &&
           laptime->doo_count > 0) {
            laptime->doo_count--;
...
static bool driver_select(Laptime* laptime, Driver_list* driver_list) {
    switch(driver_select_press.state = button_hold(
               gpio_get_level(DRIVER_SELECT_PIN), driver_select_press)) {
    case BTN_HOLD_ACTION:
        laptime->driver_id--;
...
static Wifi_reset wifi_reset_check() {
    switch(wifi_press.state =
               button_hold(gpio_get_level(WIFI_PIN), wifi_press)) {
    case BTN_HOLD_ACTION:
...
void IRAM_ATTR button_isr(Button_press* button_press) {
    ...
    if((tick - button_press->time) > pdMS_TO_TICKS(100) &&
       button_press->state == BTN_STANDBY) {
 ...
 * two_gate_mode == false - first gate saves laptime and starts new lap on every
 * negative edge two_gate_mode == true - first gate only starts new lap on
 * negative edge if stop_flag is true
 */
 ...
 * two_gate_mode == true - second gates saves laptime on negative edge if
 * stop_flag is false
 */
...
void IRAM_ATTR wifi_pin_isr() { button_isr(&wifi_press); }

#ifdef CONFIG_TWO_GATE_WIRELESS_MASTER

void IRAM_ATTR doo_pin_isr() { button_isr(&doo_press); }

void IRAM_ATTR oc_pin_isr() { button_isr(&oc_press); }

void IRAM_ATTR driver_select_pin_isr() { button_isr(&driver_select_press); }

#endif

esp_err_t isr_init() {
    ESP_ERROR_CHECK(
        gpio_isr_handler_add(LAP_GATE1_PIN, (gpio_isr_t)gate1_pin_isr, NULL));
    ESP_ERROR_CHECK(
        gpio_isr_handler_add(LAP_GATE2_PIN, (gpio_isr_t)gate2_pin_isr, NULL));
    ESP_ERROR_CHECK(
        gpio_isr_handler_add(LAP_RESET_PIN, (gpio_isr_t)reset_pin_isr, NULL));
    ESP_ERROR_CHECK(
        gpio_isr_handler_add(WIFI_PIN, (gpio_isr_t)wifi_pin_isr, NULL));

#ifdef CONFIG_TWO_GATE_WIRELESS_MASTER

    ESP_ERROR_CHECK(
        gpio_isr_handler_add(LAP_DOO_PIN, (gpio_isr_t)doo_pin_isr, NULL));
    ESP_ERROR_CHECK(
        gpio_isr_handler_add(LAP_OC_PIN, (gpio_isr_t)oc_pin_isr, NULL));
    ESP_ERROR_CHECK(gpio_isr_handler_add(
        DRIVER_SELECT_PIN, (gpio_isr_t)driver_select_pin_isr, NULL));
 ...
 * 5. When laptime is saved, stores it locally and sends through UART, sd card,
 * shows it on LCD and WIFI page
 */
void laptimer_task(void* args) {
    ...
    for(;;) {
        ...
        driver_select(&laptime_current, &config_local.driver_list);
        ...
        if(laptime_saved.time > 0) {
            if(xSemaphoreTake(laptime_lists_mutex, 0) == pdTRUE) {
                ...
                ESP_ERROR_CHECK(system_get_time(laptime_saved_local.timeofday,
                                                laptime_saved_local.date));
                // ESP_ERROR_CHECK(rtc_set_time(laptime_saved_local.timeofday,
                // laptime_saved_local.date));
                ESP_LOGI(TAG, "TIMEOFDAY: %s, DATE: %s",
                         laptime_saved_local.timeofday,
                         laptime_saved_local.date);
